    ...
    <div v-show="cartState.open" class="cart-overlay">
      <transition name="fade">
        <div
          ...
          class="cart-overlay-background overflow-hidden"
        ></div>
      </transition>
          ...
          <div v-show="cartState.open" ref="Cart" class="cart-panel">
            ...
            <div class="cart-header">
              ...
              <button @click="$emit('close')">
                <span class="sr-only">{{ $t("components.cart.close") }}</span>
                <X strokeWidth="1" />
              </button>
            </div>
                ...
                <div v-if="cartTotalPrice() >= 100">
                  <div class="cart-eligible">
                    ...
                    <div class="cart-eligible-details">
                      ...
                      <p>{{ $t("components.cart.yourItems") }}</p>
                    </div>
                  </div>
                  <!-- Free Shipping Bar -->
                  <div class="cart-shipping">
                    <div class="cart-shipping-info">
                      ...
                      <div class="cart-shipping-icon">
                        <hr class="cart-shipping-divider" />
                        <span class="cart-shipping-circle">
            ...
            <div class="cart-footer">
              <div class="cart-total">
                <div class="cart-total-label">
                  <h6>
                    <strong>{{ $t("components.cart.total") }}</strong>
                  </h6>
                  <p class="cart-total-tax">
                    {{ $t("components.cart.inclTax") }}
                  </p>
                </div>
                <div>{{ cartTotalPrice() }} {{ $currency }}</div>
              </div>
              <router-link
                to="/checkout"
                ...
                @click="
                  () => {
                    if (cartState.open) cartState.open = false;
                  }
                "
        ...
        if (cartProduct.items) {
          cartProduct.items = cartProduct.items
            .map((cartItem) => {
              ...
              if (!validItem) {
                toast.warning(
                  t("components.cart.item_invalid", { id: cartItem.id })
                );
                return null;
              }
