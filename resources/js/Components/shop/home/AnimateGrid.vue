<template>
  <div :class="cn('relative hidden xl:block', props.class)">
    ...
    >
      ...
      >
        <router-link :to="item.link" @click.stop class="cursor-pointer" 
        data-aos="zoom-in"
        :data-aos-delay="600 * (index + 1)">
          <slot name="logo" :logo="item.logo" :index="index" />
          <!-- <strong class="block text-center mt-2 text-accent_dark">{{ item.name }}</strong> -->
        </router-link>
      </div>
    </div>
  </div>
...
.card {
  box-shadow: 2px 2px 5px rgba(217, 251, 232, 0.5),
    3px 3px 10px rgba(217, 251, 232, 0.5), 6px 6px 20px rgba(217, 251, 232, 0.1);
}
...
.dark .card:hover {
  box-shadow: 3px 3px 5px rgba(31, 41, 55, 1), 5px 5px 10px rgba(31, 41, 55, 1),
