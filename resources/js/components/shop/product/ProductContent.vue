<template>
  <div class="grid w-full gap-4 md:w-2/3 lg:w-3/4 grid-cols-[3fr_1fr]">
    <!-- Product Image Carousel -->
    <div class="relative w-full overflow-hidden rounded-lg shadow-xl aspect-square bg-gray-100"
    ...
    >
      ...
      />
      <div class="absolute bottom-4 right-4 text-gray-700 cursor-pointer">
    ...
    <!-- Thumbnail Images -->
    <div class="flex flex-col gap-4 overflow-x-auto md:overflow-y-auto w-full md:w-auto">
      <div
        v-for="(image, index) in imageUrls"
        :key="index"
        @click="selectImage(index)"
        class="flex-shrink-0 cursor-pointer p-1 rounded-lg transition-all duration-200"
        :class="{ 'border-2 border-black shadow-lg': index === activeIndex }"
      >
        <img
          :src="image"
...
const imageUrls = computed(() =>
  props.data?.items?.filter((item) => item.image_url).map((item) => item.image_url) || []
);
