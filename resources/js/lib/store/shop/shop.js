  ...
  const cartState = useStorage(
    ...
    undefined,
    {
      serializer: {
        read: (v) => {
          try {
            ...
            return {
              ...
              discounts: [],
              ...data,
              promoCode: "", // Always start with an empty promo code on load
            };
          } catch (e) {
            return {
              open: false,
        ...
        write: (v) => {
          const { promoCode, ...data } = v; // Exclude promoCode from being stored
      ...
      for (const cartProduct of cartState.value.cartItems) {
        try {
          ...
          if (!validProduct || validProduct.id !== cartProduct.id) {
            ...
            cartState.value.cartItems = cartState.value.cartItems.filter(
