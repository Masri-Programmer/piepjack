        ...
        <table class="w-full my-0 align-middle text-dark border-neutral-200">
          <thead class="align-bottom">
            <tr class="font-semibold text-[0.95rem] text-gray">
              ...
              <th class="pb-3 px-1 text-start uppercase">ORDER ID</th>
              <th class="pb-3 px-1 text-start uppercase min-w-[120px]">
                STATUS
              </th>
              <th class="pb-3 px-1 text-start uppercase">REASON</th>
          ...
          <tbody>
            ...
            <template v-if="!isLoading && data?.data">
              ...
              >
                    ...
                    <div class="flex flex-col justify-start w-full">
                      ...
                      >
                        <select
                          ...
                          :class="[
                            'font-bold',
                            returning.status === 'not_requested'
                              ? 'text-green-500'
                              : returning.status === 'requested'
                              ? 'text-yellow-700'
                          ...
                          @change="
                            (e) =>
                              handleUpdate(returning, 'status', e.target.value)
                          "
                ...
                <td class="p-3 text-start text-sm text-pretty">
                  {{ returning.reason }}
                </td>
                <td class="p-3 text-start">
                  <span
                    ...
                    >{{ formatDate(returning.created_at) }}</span
                  >
                </td>
                <td class="p-3 text-start">
                  <span
                    class="text-xs text-center align-baseline inline-flex p-3 mr-auto items-center font-semibold text-[.95rem] leading-none text-blue-500 bg-blue-500/20 rounded-lg"
                    >{{ formatDate(returning.updated_at) }}</span
                  >
                </td>
                <td class="p-3 text-start">
                  <div class="flex items-center gap-2">
                    <button
                      ...
                      @click="() => handleDelete(returning.id)"
                    >
