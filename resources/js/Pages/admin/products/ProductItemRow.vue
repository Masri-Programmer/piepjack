<template>
  <tr class="border-b border-dashed last:border-b-0 border-main">
    ...
    <td class="p-3">
      <div class="relative inline-block shrink-0 rounded-2xl me-3">
        <ProductImg
          ...
          @handle:img="(data) => Object.assign(newProductItem, data.value)"
        />
      </div>
    </td>
    ...
    <template v-for="v in variations?.data" :key="v.id">
      ...
      >
        ...
        >
          ...
          <option v-for="vo in v?.options" :key="vo.id" :value="vo.id">
            {{ vo.value }}
          </option>
        </select>
      </td>
    ...
    <td class="pr-0 text-start" v-if="isLoadingVariations">
      ...
      >
        <div class="w-full h-10 bg-gray rounded"></div>
      </span>
    </td>
    <td class="pr-0 text-start" v-if="isLoadingVariations">
      <span
...
const { data: variations, isLoading: isLoadingVariations } =
  apiQuery("category-variations").useGetById(props.product?.category?.id);
